...
public partial class Inventory_PurchasePrint : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        try
        {
            salesManID = Int32.Parse(Request.QueryString["SalesManID"]);
        }
        catch (Exception ex)
        {
            salesManID = 0;
...
,SalesPerson.ChartOfAccountLabel4Text as SalesPersonName 
...
inner join ACC_ChartOfAccountLabel4 as SalesPerson on SalesPerson.ACC_ChartOfAccountLabel4ID= Pos_TransactionMaster.Record
where   Pos_TransactionMaster.RowStatusID=1 and Pos_TransactionMaster.Pos_TransactionTypeID =" + TransactionTypeID.ToString() + @" and  (Pos_TransactionMaster.TransactionDate between '" + fromDate + "' and '" + toDate + "')";
        if (suppliyerID != 0)
        {
        ...
        if (salesManID != 0)
        {
            sql += " and SalesPerson.ACC_ChartOfAccountLabel4ID =" + salesManID;
        }
...
,ACC_ChartOfAccountLabel4.ExtraField1
,Pos_Product.BarCode,Pos_Product.ProductID
,SalesPerson.ChartOfAccountLabel4Text
,Pos_TransactionMaster.TransactionDate 
order by SalesPerson.ChartOfAccountLabel4Text,Pos_Product.ProductID,Pos_Product.BarCode;";
        ...
        DataSet ds= CommonManager.SQLExec(sql);
            ...
            string lastSalesPersonName = "";

            foreach (DataRow dr in ds.Tables[0].Rows)
            {
               
                if (lastSalesPersonName == "")
                {
                    htmlTable += @"<tr>
                            <td colspan='7' style='background-color:gray;padding-left:20px;font-weight:bold;font-size:15px; border-top:1px solid black;border-bottom:1px solid black;'>
                                S.P.: " + dr["SalesPersonName"].ToString() + @"
                            </td>
                        </tr>";
                    Subtotal = 0;
                    SubtotalAmount = 0;
                    Total = 0;
                    TotalAmount = 0;
                    lastSalesPersonName = dr["SalesPersonName"].ToString();

                }
                else
                if (lastSalesPersonName != dr["SalesPersonName"].ToString())
                {
                ...
                grandTotal += decimal.Parse(dr["Quantity"].ToString());
                grandTotalAmount += decimal.Parse(dr["TotalAmount"].ToString());
            }
            ...
            htmlTable += @"<tr id='lastRow'>
                        ...
                        <td>&nbsp;</td>
                        <td>Grand Total</td>
                        <td>" + grandTotal.ToString("0,0.00") + @"</td>
                        <td>&nbsp;</td>
                        <td>" + grandTotalAmount.ToString("0,0.00") + @"</td>
                    </tr></table>";
