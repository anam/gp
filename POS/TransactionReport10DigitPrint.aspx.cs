...
public partial class Inventory_PurchasePrint : System.Web.UI.Page
{
    ...
    private void update10DigitCode()
    {
        string sql = "Select [Pos_ProductID],[BarCode] from [Pos_Product] where [Pic2]=''";

        DataSet ds = CommonManager.SQLExec(sql);
        sql = "";
        foreach (DataRow dr in ds.Tables[0].Rows)
        {
            string _10DigitCode=dr["BarCode"].ToString();
            if (_10DigitCode.Length > 10)
            {
                _10DigitCode = _10DigitCode.Substring(0, 10);
            }

            sql += @"
            Update Pos_Product set Pic2='" + _10DigitCode + "' where Pos_ProductID=" + dr["Pos_ProductID"].ToString() + ";";
        }
        if(sql!="")
...
,SUM((Pos_Transaction.Quantity * Pos_Product.SalePrice)+(Pos_Transaction.Quantity * Pos_Product.SalePrice * [Pos_Product].[VatPercentage] /100)) as TotalAmount
,(SUM(Pos_Transaction.Quantity * Pos_Product.SalePrice)/SUM(Pos_Transaction.Quantity)) as UnitPrice 
,Pos_Product.Pic2 as BarCode,Pos_Product.ProductID
from
Pos_Product 
...
,ACC_ChartOfAccountLabel4.ChartOfAccountLabel4Text
,ACC_ChartOfAccountLabel4.ExtraField1
,Pos_Product.Pic2,Pos_Product.ProductID
order by Pos_Product.ProductID,Pos_Product.Pic2;";
