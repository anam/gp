        ...
        foreach (Pos_TransactionType type in allTransactionTypeRow)
        {
           try
                ...
                sql += @"--" + type.TransactionTypeName + @" Stock 
                    Select Sum(Pos_Transaction.Quantity) as Qty, Sum(Pos_Transaction.Quantity * Pos_Product.SalePrice) as QtyPrice,Pos_Product.ProductID
                    ...
                    order by Pos_Product.ProductID,ACC_ChartOfAccountLabel4.ChartOfAccountLabel4Text,ACC_ChartOfAccountLabel4.ExtraField1
        ...
        foreach (DataTable tbl in ds.Tables)
        {
            foreach (DataRow dr in tbl.Rows)
            {
                ...
                if (!isFound)
                {
                    ...
                    newProduct.ProductName = "<a href='StockReportInCentralByDateItemWisePrint.aspx?ProductID=" + dr["ProductID"].ToString() + "&FromDate=" + fromDate + "&ToDate=" + toDate + "' target='_blank'>" + dr["ChartOfAccountLabel4Text"].ToString()+"</a>";
