...
inner join Inv_QuantityUnit on Inv_QuantityUnit.Inv_QuantityUnitID=Inv_Item.QuantityUnitID
 where Inv_IssueMaster.RowStatusID=1
and Inv_IssueDetail.RowStatusID=1
and Inv_Item.ExtraField1<>'0' and  Inv_Item.RowStatusID=1 
and  Inv_IssueMaster.IssueDate <= @ToDate
";
        if (WorkStationID != 0)
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                if (RawMaterialID != "0" && RawMaterialID != dr["RawMaterialID"].ToString())
                {
                    htmlTable += @"<tr class='subtotalRow'>
                            ...
                            <td>" + StockQuantity.ToString("0,0.00") + @"</td>
                            <td>" + StockTotalAmount.ToString("0,0.00") + @"</td>
                        </tr>";
                ...
                if (decimal.Parse(dr["StockQuantity"].ToString()) == 0) continue;
