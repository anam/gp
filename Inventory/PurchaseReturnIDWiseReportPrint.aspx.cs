...
public partial class Inventory_PurchasePrint : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        //purchase info
        string sql = " and (Inv_PurchaseReturn.PurchseReturenDate between '" + fromDate + "' and '" + toDate + "')";
        if (suppliyerID != 0)
        {
            sql += " and Inv_PurchaseReturn.ExtraField1 ='" + suppliyerID+"'";
        }

        List<Inv_PurchaseReturen> purchases = Inv_PurchaseReturenManager.GetAllInv_PurchaseReturensByDateNSupplierID(sql);
         string htmlTable ="";
         foreach (Inv_PurchaseReturen purchase in purchases)
         {
             htmlTable += @"<table width='800px' style='margin-bottom:50px;'><tr>
                <td>
                    <table style='border:1px solid Black;width:100%;' > 
                        <tr>
                            <td style='width:100px;' class='PurchaseHeaderCss'>Return Date</td>
                            <td style='width:5px;'>:</td>
                            <td>
                                " + purchase.PurchseReturenDate.ToString("dd-MMM-yyyy") + @"
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>:</td>
                            <td>
                                " + purchase.Inv_PurchaseReturenID.ToString() + @"
                            </td>
                        </tr>
                        <tr>
                            <td class='PurchaseHeaderCss'>Ref. Purchase ID</td>
                            <td>:</td>
                            <td>
            ...
            <tr>
             ...
             items = Inv_ItemManager.GetAllInv_ItemsByPurchaseReturnID(purchase.Inv_PurchaseReturenID);
