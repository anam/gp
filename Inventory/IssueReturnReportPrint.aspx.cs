...
inner join Inv_IssueMasterReturn on Inv_IssueMasterReturn.Inv_IssueMasterReturnID =Inv_ItemTransaction.ExtraField5
where Inv_Item.RowStatusID=1 and Inv_IssueMasterReturn.RowStatusID=1 and Inv_ItemTransaction.RowStatusID=1 and  Inv_ItemTransaction.ItemTrasactionTypeID =3 and  (Inv_IssueMasterReturn.IssueReturnDate between '" + fromDate + "' and '" + toDate + "')";
        if (workstationID != 0)
        {
            sql += " and Inv_IssueMasterReturn.WorkSatationID =" + workstationID;
        }
